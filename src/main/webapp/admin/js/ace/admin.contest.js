jQuery.adminPontest = {
					...
					"aoColumnDefs" : [
						{
							...
							'fnRender' : function(oObj, sVal) {
								return"  <button class=\"btn2 btn-info\" onclick=\"$.adminPontest.deletePontest("+oObj.aData.id+")\"><i class=\"icon-trash\"></i> 删除</button>" +
								" <button class=\"btn2 btn-info\" onclick=\"$.adminPontest.showEdit("+oObj.aData.id+")\"><i class=\"icon-pencil\"></i>修改</button>";
							}
						},
					 {
						'aTargets' : [ '_all' ],
	            	...
	            	$.ajax({
	        			...
	        			success : function(json) {
	        				if(json.state=='success'){
	        					...
	        					$.adminPontest.initSearchDataTable();
	        				}else{
		...
		showPontestAddModal: function(id){
			$("#id").val("");
			$('#_modal').modal({
			});
			$("#_modal").modal('show');
